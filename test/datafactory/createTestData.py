...
def array2rastercsv(array, outName, templateRaster, yoffset=0, xoffset=0, DataType=gdal.GDT_Float32):
    ...
    newArr = array
    # if templateRaster.cellHeight < 0:
    #     newArr = np.flipud(array)
    ...
    with open(csvname, 'wb') as csvfile:
        ...
        for idy, row in enumerate(newArr):
            for idx, cell in enumerate(row):
                counter += 1
                left = idx * templateRaster.cellWidth + originX + templateRaster.cellWidth / 2
                top = idy * templateRaster.cellHeight + originY + templateRaster.cellHeight / 2
                spamwriter.writerow([counter, left, top, cell])
...
def main():
    ...
    for idx in range(0,squaregrid):
        for idy in range(0, squaregrid):
            folder = 'output' + path.sep + str(idx) + path.sep + str(idy) + path.sep
            ...
            topoffset = idy * (pxheight + spacing) * templateRaster.cellHeight
            leftoffset = idx * (pxwidth + spacing) * templateRaster.cellWidth
